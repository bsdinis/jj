...
fn parse_no_such_remote(stderr: &[u8]) -> Option<String> {
    ...
    let suffix = first_line
        ...
        .or(first_line.strip_prefix(b"fatal: unable to access '"))?;

    suffix
        .strip_suffix(b"' does not appear to be a git repository")
        .or(suffix.strip_suffix(b"': Could not resolve host: invalid-remote"))
...
fn parse_no_remote_tracking_branch(stderr: &[u8]) -> Option<String> {
    ...
    suffix
        ...
        .or(suffix.strip_suffix(b"' not found"))
...
fn parse_git_remote_show_default_branch(
    ...
    Ok(stdout
        ...
        .filter(|branch_name| branch_name != "(unknown)"))
}
...
fn parse_ref_pushes(stdout: &[u8]) -> Result<(Vec<String>, Vec<String>), GitSubprocessError> {
        ...
        .enumerate()
    {
        ...
        let reference = if let Some(reference) = full_refspec.split(":").last() {
            Ok(reference.to_string())
        } else {
            Err(GitSubprocessError::External(format!(
...
and the repository exists. "###;
    ...
    const SAMPLE_PUSH_REFS_ERROR: &[u8] = b"To origin
...
Done";
    ...
    fn test_parse_no_such_remote() {
        ...
        assert_eq!(parse_no_such_remote(SAMPLE_PUSH_REFS_ERROR), None);
    ...
    fn test_parse_no_remote_ref() {
        ...
        assert_eq!(parse_no_remote_ref(SAMPLE_PUSH_REFS_ERROR), None);
    ...
    fn test_parse_no_remote_tracking_branch() {
        ...
        assert_eq!(
            parse_no_remote_tracking_branch(SAMPLE_PUSH_REFS_ERROR),
            None
        );
